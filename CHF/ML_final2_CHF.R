...
# ================== 数据读取与预处理 ==================
species <- read.delim("CHF_marker_species.txt", row.names = 1, check.names = FALSE)
species_t <- as.data.frame(t(species))
species_t$Sample <- rownames(species_t)

sample_info <- read.delim("CHF_sample.txt", stringsAsFactors = FALSE)

data <- merge(species_t, sample_info, by = "Sample", all = FALSE)
data$Type <- factor(data$Type, levels = c("HC", "CHF"))
...
y_train_num <- ifelse(y_train == "CHF", 1, 0)
y_test_num  <- ifelse(y_test == "CHF", 1, 0)
...
predict_prob <- function(model, newdata, type = "response") {
  ...
  if (inherits(model, "glm")) {
    ...
    if ("CHF" %in% colnames(prob_attr)) {
      return(prob_attr[, "CHF"])
    } else {
      return(prob_attr[, 2])
    }
  } else if (inherits(model, "randomForest")) {
    return(predict(model, newdata, type = "prob")[, "CHF"])
...
plot_confusion_matrix <- function(y_true, y_pred_prob, model_name, dataset_name, threshold = 0.5) {
  ...
  y_pred <- ifelse(y_pred_prob >= threshold, "CHF", "HC")
  y_true <- ifelse(y_true == 1, "CHF", "HC")
...
if (!is.null(models$XGBoost)) {
  ...
  logit_data$y <- as.numeric(logit_data$y == "CHF")
...
bootstrap_rf <- function(X, y, B = 1000, ntree = 100) {
  ...
  for (i in 1:B) {
    ...
    pred_prob <- predict(rf_model, X_oob, type = "prob")[, "CHF"]
    pred_class <- ifelse(pred_prob >= 0.5, "CHF", "HC")
